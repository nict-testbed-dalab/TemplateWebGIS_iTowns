    ...
    const bargraphSource = new itowns.FileSource({
        ...
        parser: (buffer, options = {}) => {
            ...
            for (let i = 0; i < parsed.data.length; ++i) {
                ...
                material.opacity = 1.0;
                const geo = new THREE.CylinderGeometry( 2, 2, 1, 32 );
                geo.translate(0, 0, -0.5);
...
// グラフを表示する関数？
export function CreateBargraphLayer(itownsView, config, num=0) {
            ...
            this.source.loadData(this.BarGraphExtent, this).then((data) => {
                ...
                for (let i = 0; i < data.meshGroup.children.length; ++i) {
                    ...
                    if (!this.itownsView.isPlanarView) {
                        ...
                        mesh.lookAt(new THREE.Vector3(zeroVector.x, zeroVector.y, zeroVector.z));
                    }
                    ...
                    if (mesh.visible && isValidTimeIndex && this.currentDate) {
                        ...
                        if (this.range) {
                            // 現在時刻がレンジ範囲外なら非表示とする
                            if (this.currentDate< this.range.rangeStartTime || this.currentDate > this.range.rangeEndTime) {
                                mesh.visible = false;
                            }
                            // データ時刻がレンジ範囲外なら非表示とする
                            if (date.getTime() < this.range.rangeStartTime || date.getTime() > this.range.rangeEndTime) {
                                mesh.visible = false;
                            }
                        }
                    }
