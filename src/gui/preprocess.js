...
export function setupPreprocess(view) {
    ...
    $('#btn_prepro_get_data').on('click', async function() {
        ...
        const v_camera = view.camera.camera3D;
        const p = itowns.CameraUtils.getTransformCameraLookingAtTarget(view, v_camera);
        const center = p.coord;
        let lng,lat = null;
        if(center != undefined){
            lng = center['x'];
            lat = center['y'];
        ...
        let extent = view.tileLayer.info.displayed.extent;
       
        let point_1 = extent.east + "," + extent.north;
        let point_2 = extent.east + "," + extent.south;
        let point_3 = extent.west + "," + extent.south;
        let point_4 = extent.west + "," + extent.north;
        ...
        try{
            ...
            if (response.ok) {
                ...
                if (prepro_date_int_list[source_id].length > 0){
                    ...
                    // レンジバー設定
                    // ***********************************
                    // fncSetRange(range_startTime.getTime(), range_endTime.getTime());
                    ...
                    result_inner.innerHTML = "ファイルをドラッグ＆ドロップしてください";
                    ...
                    temp_response_json.map_center = temp_response_json.data_array[0].features[0].geometry.coordinates[0][0] + "," + temp_response_json.data_array[0].features[0].geometry.coordinates[0][1];
                    temp_response_json.map_pitch = 25;
                    temp_response_json.map_bearing = 0;
                    
                    if(source_id === "layer_amedas") {
                        setupAmedasJson(temp_response_json);
                    } else if(source_id === "layer_garbagetruck") {
                        setupJson(temp_response_json);
                    }
    ...
    $("#feature_size").change(function(){
        ...
        let size_value = $("#feature_size").val();
        ...
        if("mesh" in view) {
            ...
            objs.traverse((m) => {
                if(m.type == "Mesh") {
                    let obj = m;
                    obj.scale.setScalar(size_value);
    ...
    // 「スプライン補間」「線形補間」の時だけ時間粒度のプルダウンメニューを変更
    document.getElementById("proc_type").addEventListener("change", () => {
        let options;
        $('select#granularity option').remove();

        // アメダスの時
        if($("#prepro_db_data_type").val() === "layer_amedas") {
            if($("#proc_type").val() === "spline" || $("#proc_type").val() === "linear") {
...
function updatePreproJsonLayer(dateTimeNum, is_new){
    ...
    for(let i=0;i<self_prepro_source_ids.length;i++){
        ...
        if (closest_date === undefined){
            ...
            console.log("日時が範囲外");
            if (isRangeActive() == false){
                // 範囲設定が無効の場合(範囲指定していれば、クリアはしない（自動的に範囲内に戻るため）)
                // ※wgapp.mapはitownsように修正必要
                if (wgapp.map.getSource(source_id)) {
                    // 存在しない日時でデータ非表示
...
// /**
//  * 日付の範囲を設定する（主に子ウィンドウから呼び出すための関数）
//  * @param {*} startTime  開始日時(Timestamp型)
//  * @param {*} endTime    終了日時(Timestamp型)
//  */
// function fncSetRange(startTime, endTime){
//     console.log("---fncSetRange---");

//     const diffTime = Math.abs(startTime - endTime);
//     const diffDay = Math.floor(diffTime / (1000 * 60 * 60 * 24));

//     // 日付zoomを設定する 
//     // 7: 1month, 8 : 15day, 9: 1day
//     let zoomValue = 0;
//     if(diffDay <= 0.75) {
//         zoomValue = 11; // 12hour
//     }else if(diffDay <= 1.25) {
//         zoomValue = 10; // 1day
//     }else if(diffDay <= 10){
//         zoomValue = 9; // 7day
//     }else if(diffDay <= 20){
...
//     }else if(diffDay <= 400){
//         zoomValue = 4; // 1year
//     }else if(diffDay <= 2000){
//         zoomValue = 3; // 5year
//     }else if(diffDay <= 4000){
//         zoomValue = 2; // 10year
//     }else{
//         zoomValue = 1;
//     }

//     var objZoomInfo = $Env.zoomTable[zoomValue];
//     var intPixelSize;

//     intPixelSize = objZoomInfo.value / (endTime - startTime);
...
//     // 見た目の範囲(rangeではない)
//     objTimeInfo.startTime    = new Date(startTime - intPixelSize * 0.25 * ( (endTime - startTime)/2 ));
//     objTimeInfo.endTime      = new Date(endTime   + intPixelSize * 0.25 * ( (endTime - startTime)/2 ));

//         // 開始日時にする
//     objTimeInfo.currentTime  = new Date(startTime);
...
//     if ($("#button_range").hasClass("active") == false){
//         $("#button_range").trigger("click");
//     }
//     $("#timeline").k2goTimeline("showRangeBar", {
//             rangeStartTime : new Date(startTime),   // startTime
//             rangeEndTime : new Date(endTime),       // endTime
//         });
...
//     return $("#button_range").hasClass("active");
// }
