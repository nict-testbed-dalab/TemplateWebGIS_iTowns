    ...
    function fillToolTip(features, layer, options) {
        ...
        for (var p = 0; p < features.length; p++) {
            ...
            if (geometry.properties) {
                content += (geometry.properties.name || geometry.properties.nom || geometry.properties.description || geometry.properties.N10_004 ||layer.name || '');
            }
            ...
            if (geometry.properties && !options.filterAllProperties) {
                if (options.format) {
                    ...
                    for (prop in geometry.properties) {
                        if (!options.filterProperties.includes(prop)) {
                            if(prop != "style") {
                                content += '<li style="list-style:none;padding-bottom:2px;margin-right:4px;">' + prop + ': ' + geometry.properties[prop] + '</li>';
                            }
                        }
                    }
